...
class CommonBaseClient(object):
    ...
    def _http_response(self, url, method, data=None, **kwargs):
        ...
        response = method(
            self.host + path, data=data, headers=header, **self.method_kwargs
        )
...
class BaseClientV1(CommonBaseClient):
    ...
    def put_images_layer(self, image_id, data):
        ...
        return self._http_call(
            self.IMAGE_LAYER, put, image_id=image_id, data=data
        )

    def put_image_layer(self, image_id, data):
        """PUT /v1/images/(image_id)/json"""
        return self._http_call(
            self.IMAGE_JSON, put, data=data, image_id=image_id
        )
    ...
    def get_repository_tags(self, namespace, repository):
        ...
        return self._http_call(
            self.TAGS, get, namespace=namespace, repository=repository
        )
    ...
    def delete_repository(self, namespace, repository):
        ...
        return self._http_call(
            self.REPO, delete, namespace=namespace, repository=repository
        )
...
class BaseClientV2(CommonBaseClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Default to the main part of the repository hostname if the
        # service name is missing or None (the default)
        auth_service_name = (
            kwargs.pop("auth_service_name", "") or urlsplit(host).netloc
        )

        # Get the URL of the auth service from the args, accounting
        # for the deprecated url arg
        ...
        if deprecated_auth_service_url_arg:
            warnings.warn(
                (
                    "The auth_service_url argument is deprecated; use "
                    "auth_service_url_full instead"
                ),
                DeprecationWarning,
            )
            if not auth_service_url:
                auth_service_url = urljoin(
                    deprecated_auth_service_url_arg, "v2/token"
                )
        ...
        # itself.
        #
        # We must pop the auth kwarg so it does not get sent to
        # requests, because override the authentication token if it
        # sees the username/password provided
        #
        # See: http://docs.python-requests.org/en/master/user/quickstart/#custom-headers # noqa: E501
        if auth_service_url:
    ...
    def delete_manifest(self, name, digest):
        self.auth.desired_scope = "repository:%s:*" % name
        return self._http_call(
            self.MANIFEST, delete, name=name, reference=digest
        )
    ...
    def copy_blob(self, origin, digest, destination):
        self.auth.desired_scope = [
            "repository:%s:*" % repo for repo in (origin, destination)
        ]
        return self._http_call(
            self.BLOB_MOUNT,
            post,
    ...
    def _http_response(
        self, url, method, data=None, content_type=None, schema=None, **kwargs
    ):
        """url -> full target url
           method -> method from requests
           data -> request body
