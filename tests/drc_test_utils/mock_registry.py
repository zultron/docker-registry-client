...
class MockV2Registry(MockRegistry):
    ...
    BLOB_MOUNT = MockRegistry.format(
        "/v2/{name2}/blobs/uploads/?mount={digest}&from={name}"
    )
    ...
    GET_MAP = {
        ...
        TAGS_LIBRARY: MockResponse(
            200, data={"name": TEST_NAME, "tags": [TEST_TAG]}
        ),
        MANIFEST_TAG: MockResponse(
            200,
            data={"name": TEST_NAME, "tag": TEST_TAG, "fsLayers": []},
            headers={"Docker-Content-Digest": TEST_MANIFEST_DIGEST,},
        ),
    }
