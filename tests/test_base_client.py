...
class TestBaseClientV2(object):
    ...
    def test_get_manifest_and_digest(self):
        client = BaseClientV2(self.url)
        manifest, digest = client.get_manifest_and_digest(TEST_NAME, TEST_TAG)
    ...
    def test_auth_url_defaults(self):
        assert (
            BaseClientV2(
                self.url, auth_service_url="https://myhost.com"
            ).auth.url
        ...
        assert (
            BaseClientV2(
                self.url, auth_service_url_full="https://myhost.com/foo"
            ).auth.url
            == "https://myhost.com/foo"
        )

    def test_copy_blob(self):
        BaseClientV2(self.url).copy_blob(
            TEST_NAME, TEST_MANIFEST_DIGEST, TEST_NAME2
        )
