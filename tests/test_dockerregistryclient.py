...
class TestDockerRegistryClient(object):
    ...
    def test_repository_namespace_incorrect(self, version):
        ...
        with pytest.raises(RuntimeError):
            client.repository(
                "{0}/{1}".format(TEST_NAMESPACE, TEST_REPO),
    ...
    @pytest.mark.parametrize(
        ("client_api_version", "registry_api_version", "should_succeed"),
        [(1, 1, True), (2, 2, True), (1, 2, False), (2, 1, False),],
    )
    def test_api_version(
        self, client_api_version, registry_api_version, should_succeed
    ):
        ...
        if should_succeed:
            ...
            with pytest.raises(HTTPError):
                client = DockerRegistryClient(
                    url, api_version=client_api_version
                )
                client.refresh()
